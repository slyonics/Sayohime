...
namespace Sayohime.SceneObjects
{
	...
	public class ParallaxBackdrop
	{
		...
		public ParallaxBackdrop(string backdropName, float[] parallaxSpeeds)
		{
			...
			{
				Texture2D backgroundSprite = AssetCache.SPRITES[Enum.Parse<GameSprite>($"Background_{backdropName}_{backdropName}{i}")];
		...
		public void Draw(SpriteBatch spriteBatch)
		{
			...
			foreach (Layer layer in layerList)
			{
				...
				{
					for (int i = 0; i < Math.Max(2, CrossPlatformGame.SCREEN_WIDTH / layer.background.Width); i++)
					{

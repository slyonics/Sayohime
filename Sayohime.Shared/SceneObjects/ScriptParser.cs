...
namespace Sayohime.SceneObjects
{
	...
	public class ScriptParser
	{
			...
			if (!scriptedController.ExecuteCommand(tokens))
			{
				switch (tokens[0])
				{
                    ...
                    case "ScreenShake":
						...
						{
							CrossPlatformGame.ScreenShake = new Vector2(0, (float)Math.Clamp(Math.Sin(amount * Math.PI * 2) * 6, -4, 4));
						});
						break;
				}
			}
		...
		private string ParseParameter(string parameter)
		{
			...
			else
			{
				...
				else
				{
					switch (parameter)
					{
						case "$centerX": return (CrossPlatformGame.SCREEN_WIDTH / 2).ToString();
						case "$centerY": return (CrossPlatformGame.SCREEN_HEIGHT / 2).ToString();
						case "$right": return CrossPlatformGame.SCREEN_WIDTH.ToString();
		...
		private void If(string[] tokens)
		{
			...
			if (!EvaluateConditional(string.Join(' ', tokens.Skip(1))))
			{
				...
				do
				{
					...
					if (skipLine.StartsWith("If") || skipLine.StartsWith("Switch")) nestLevel++;
		...
		private void SetProperty(string[] tokens)
		{
			...
			(GameProfile.CurrentSave.GetType().GetProperty(tokens[1]).GetValue(GameProfile.CurrentSave) as ModelProperty<string>).Value = propertyValue;
		}

	...
	public class Tooltip : Widget
	{
		...
		public Tooltip(Vector2 mouseOverPosition, string tooltipText)
		{
			...
			while (startIndex != -1 && endIndex > startIndex)
			{
				...
				PropertyInfo propertyInfo = GameProfile.CurrentSave.GetType().GetProperty(originalToken.Substring(1, originalToken.Length - 2));
				string newToken = (propertyInfo.GetValue(GameProfile.CurrentSave) as ModelProperty<string>).Value;
			...
			if (currentWindow.Right > CrossPlatformGame.SCREEN_WIDTH) currentWindow.X = (int)mouseOverPosition.X - TOOLTIP_MARGIN_WIDTH + TOOLTIP_MARGIN_WIDTH - Math.Max(Text.GetStringLength(Font, text) + TOOLTIP_MARGIN_WIDTH * 2, tooltipFrame.FrameWidth * 3);
			if (currentWindow.Bottom > CrossPlatformGame.SCREEN_HEIGHT) currentWindow.Y -= currentWindow.Bottom - CrossPlatformGame.SCREEN_HEIGHT;

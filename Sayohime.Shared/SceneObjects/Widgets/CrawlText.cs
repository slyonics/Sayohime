	...
	public class CrawlText : Widget
	{
		...
		public override void Draw(SpriteBatch spriteBatch)
		{
			...
			if (!Transitioning)
			{
				foreach (TextElement textElement in textLines)
				{
					...
					{
						spriteBatch.Draw(textElement.icon, base.Position + new Vector2(currentWindow.X + textElement.offset - 2, currentWindow.Y + textElement.line * Main.Text.GetStringHeight(Font)), null, Color.White, 0.0f, Vector2.Zero, 1.0f, SpriteEffects.None, 0.05f);
					}
		...
		private List<TextElement> ExpandText(string text)
		{
			...
			while (startIndex != -1 && endIndex > startIndex)
			{
				...
				string newToken;
				switch (originalToken.Substring(1, originalToken.Length - 2))
				{
					case "PlayerNameEx":
		...
		private List<TextElement> GetTextLines(List<TextElement> textElements, int windowWidth)
		{
			...
			foreach (TextElement textElement in textElements)
			{
				...
				{
					if (currentLength + 8 > windowWidth)
					{
						currentLine++;
						currentLength = 0;
					}
					...
					textLines.Add(new TextElement() { line = currentLine, offset = currentLength, color = currentColor, icon = AssetCache.SPRITES[gameSprite] });
					currentLength += 8 + Main.Text.GetStringLength(Font, " ");
					continue;
				}

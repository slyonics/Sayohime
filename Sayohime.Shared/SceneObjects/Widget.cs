...
namespace Sayohime.SceneObjects
{
	...
	public abstract class Widget : Overlay
	{
		...
		protected Rectangle Bounds { get => bounds; set => bounds = value; }
		...
		protected virtual int ParseInt(string token)
		{
			...
			else if (token[0] == '$')
			{
				switch (token)
				{
					case "$ScreenWidth": return CrossPlatformGame.SCREEN_WIDTH;
					case "$ScreenHeight": return CrossPlatformGame.SCREEN_HEIGHT;
					case "$CenterX": return CrossPlatformGame.SCREEN_WIDTH / 2;
					case "$CenterY": return CrossPlatformGame.SCREEN_HEIGHT / 2;
					case "$Top": return 0;
					case "$Bottom": return CrossPlatformGame.SCREEN_HEIGHT;
		...
		protected virtual string ParseString(string token)
		{
			...
			else
			{
				switch (tokens[0])
				{
					case "$PlayerProfile": return (GameProfile.CurrentSave.GetType().GetProperty(tokens[1]).GetValue(GameProfile.CurrentSave) as ModelProperty<string>).Value;
					case "$Party": return GameProfile.CurrentSave.Party[int.Parse(tokens[1])].Name.Value;
				}
			}
		...
		public T LookupBinding<T>(string bindingName, List<IModelProperty> rootDependencies = null) where T : class
		{
			...
			{
				case "PlayerProfile":
					dataContext = GameProfile.CurrentSave;
		...
		protected GameFont Font { get; set; } = GameFont.Interface;
